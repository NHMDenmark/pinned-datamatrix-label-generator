    ...
    return Label(
        ...
        text_lines=[
            Text(
                ...
                alignment="left",
                orientation="top",
                margins=(2,0.5,0.5,0.5), # mm offset from (top, right, bottom, left)
                font_size=3.55
            ),
            Text(
                ...
                alignment="left",
                orientation="top",
                margins=(4,0.5,0.5,0.5),
                font_size=2.55
            )
        ],
        check_overlap=True,
        dot_alignment=None, # "top_center", # when set to None, the dot will not be printed
        dot_offset=(0, 2.5), # right offset, top offset
...
def NHMA(number: int, bottom_text: str) -> Label:
    return Label(
        data=str(number).zfill(9),
        width=12,  # label width in mm
        height=5,   # label height in mm
        font_size=1,
        text_lines=[
            Text(
                text="NHMA",
